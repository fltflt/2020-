...
static _edge_T forwardG[MAX_RECORDS];         // 图，用一维前向星结构存储

static int head_next[MAX_NODES];		// 图相关，记录每一个节点的邻接点在图中的位置
static _degree degrees[MAX_NODES];				// 出入度信息
static _centraility centrality[MAX_NODES];		// 节点关键信息
static int special_node_flag[MAX_NODES];		// 记录特殊节点的相关信息
...
struct Graph {
	...
	void bfs_reset_id(const int st, int& k, bool* visit) {
		...
		while (!qu.empty()) {
			...
			for (int i = head_next[now], j = head_next[now + 1]; i < j; i++) {
	...
	void set_head_info() {
		...
		offset = 0;
		for (int i = 0; i < n_nodes; ++i) {
			head_next[i] = offset;//{ offset, offset + degrees[i].out_degree };
			offset += degrees[i].out_degree;
		}
		head_next[n_nodes] = offset;

		// 拷贝图
		if (max_edge < 2000 && n_nodes < 300000) {
			data_type_flag = 32;
		}
		else {
			data_type_flag = 64;
		...
		for (int i = 0; i < n_records; i++) {
			forwardG[i].to = forwardG_org[i].to;
			forwardG[i].money = forwardG_org[i].money;
		}
	}
	...
	void statistical_special_nodes() {
		// 初始化特殊节点信息
		memset(special_node_flag, 0, sizeof(int) * n_nodes);

		for (int i = 0; i < n_nodes; i++) {
			if (degrees[i].in_degree == 0 && degrees[i].out_degree == 1) {
				auto& edge = forwardG_org[head_next[i]];
				// flag 值为1表示该节点出度为1，入度为0
				special_node_flag[edge.from] = 1;

				// flag 值为2表示特殊节点的邻接节点
				if (special_node_flag[edge.to] == 0) {
					special_node_flag[edge.to] = 2;
				}
				else {
					special_node_flag[edge.to]++;
				}
			}
			else if (degrees[i].out_degree == 0) {
				//auto& edge = forwardG_org[head_next[i]];
				special_node_flag[i] = 1;
			}
		}
	}
...
struct Thread_DIJ
{
	...
	//std::unordered_map<uint64, std::vector<ui>> map_dist;
	//std::priority_queue<uint64, std::vector<uint64>, std::greater<uint64>> Q_set;
	VMap_uint32<uint32, T> vmap;
...
Thread_DIJ_uint64<uint64> dij_threads_int64[NUM_THREADS];
...
void run_one_node(const uint32 begin, Thread_DIJ<T>& pthred) {
	 ...
	 * --------------------------------------------------------------------------------
	 */
	if (begin >= n_nodes || special_node_flag[begin] == 1) {
		return;
	}
	...
	//std::unordered_map<uint64, std::vector<ui>>& map_dist = dij_threads_int64[pid].map_dist;
	//std::priority_queue<uint64, std::vector<uint64>, std::greater<uint64>>& Q_set = dij_threads_int64[pid].Q_set;
	 ...
	 */
	int coef = 1;
	if (special_node_flag[begin] > 1) {
		coef = special_node_flag[begin];
...
*/
template <class T>
void run_one_node_64(const uint32 begin, Thread_DIJ_uint64<T>& pthred) {
 ...
 */
template <class T>
void initial_64(Thread_DIJ_uint64<T>& idij) {
	int offset = 0;
	for (int i = 0; i < n_nodes; i++) {
...
template <class T>
void initial_32(Thread_DIJ<T>& idij) {
	int offset = 0;
	for (int i = 0; i < n_nodes; i++) {
		auto& idis = idij.dis_set[i];
...
void run_one_group_int64(const int& igroup, const int pid) {
	...
	for (int i = info.start; i < info.end; i++) {
		run_one_node_64(i, dij_threads_int64[pid]);
	}
...
inline void run_one_group_int32(const int& igroup, const int pid) {
	...
	for (int i = info.start; i < info.end; i++) {
		run_one_node(i, dij_threads_int32[pid]);
	}
...
void run_one_thread(int pid) {
	...
	case 64:
		// 首先给结构体初始化
		initial_64(dij_threads_int64[pid]);
	...
	case 32:
		// 首先给结构体初始化
		initial_32(dij_threads_int32[pid]);
